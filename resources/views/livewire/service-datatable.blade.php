<div>
    ...
    @elseif($createMode)
      ...
      @else
            ...
            <a class="flex items-center justify-between px-4 py-2 text-sm font-medium leading-5 text-white transition-colors duration-150 border border-transparent bg-blue-600 rounded-lg active:bg-blue-600  focus:outline-none focus:shadow-outline-blue float-right cursor-pointer"  wire:click="create()">
    ...
    <div class="row mb-4 text-gray-600 dark:text-gray-300">         
        <div class="flex space-x-2" style="display: inline-flex;">
         ...
         <div class="col float-right" style="margin-top: 5px; border: 2px solid #a5a7a7; border-radius:5px">
                ...
                <tbody class="bg-transperent divide-y dark:divide-gray-700">
                    @foreach ($services as $service)
                        <tr class="text-xs font-semibold tracking-wide text-left text-gray-500 border-b dark:border-gray-700 bg-gray-50 dark:text-gray-400 dark:bg-gray-800">
                            <td class="px-4 py-3 break-all"> {{ $service->id }} </td>
                            <td class="break-all"> {{ $service->name }} </td>
                            <td class="px-4 py-3">  @if($service->icon != null)<img src="{{ $service->icon }}" width="100" height="100" />@else Not uploaded @endif</td>
                            <!-- <td class="px-4 py-3"><textarea disabled width="100" height="150"> {{ $service->detail }} </textarea> </td> -->
                            <td>
                                <div class="flex space-x-1">&nbsp;&nbsp;
                                  <button wire:click="edit({{ $service->id }})" id="edit" class="p-1 text-blue-600 hover:bg-blue-600 hover:text-white rounded">
                                        <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path d="M13.586 3.586a2 2 0 112.828 2.828l-.793.793-2.828-2.828.793-.793zM11.379 5.793L3 14.172V17h2.828l8.38-8.379-2.83-2.828z"></path></svg>
                                  </button>
                                  
                                    <button wire:click="confirmDelete({{ $service->id }})" class="deleteService p-1 text-red-600 hover:bg-red-600 hover:text-white rounded">
              ...
              <div class="dark:bg-gray-800 p-2 sm:flex items-center justify-between">
                ...
                <div class="dark:bg-gray-800 my-4 sm:my-0">
                  <div  class="dark:bg-gray-800 lg:flex justify-center">
                      <span class="dark:bg-gray-800">{{ $services->links('livewire.datatables.tailwind-pagination') }}</span>
                  </div>
                </div>

                <div class="flex justify-end text-gray-600 dark:bg-gray-800">
                Services {{ $services->firstItem() }} - {{ $services->lastItem() }} of
                  {{ $services->total() }}
                </div>
              </div>
            </div>
          ...
          <div class="h-auto p-4 mx-2 text-left bg-white rounded shadow-xl md:max-w-xl md:p-6 lg:p-8 md:mx-0" @click.away="open = false">
            ...
            <div class="mt-5 sm:mt-6">
              <span class="flex rounded-md shadow-sm">
                ...
                <button wire:click="serviceDelete()" class="inline-flex ml-4 justify-center w-full px-4 py-2 text-white bg-red-500 rounded hover:bg-red-700">
                    Delete!
                </button>
              </span>
            </div>
