<div>
    @if($updateMode)
      @include('admin.servicecategory.edit')
    @elseif($createMode)
      @include('admin.servicecategory.create')
      @else
   
            <a class="flex items-center justify-between px-4 py-2 text-sm font-medium leading-5 text-white transition-colors duration-150 border border-transparent bg-blue-600 rounded-lg active:bg-blue-600 hover:bg-blue-700 focus:outline-none focus:shadow-outline-blue float-right cursor-pointer"  wire:click="create()">
            ...
            <table class="w-full table-fixed data-table dark:text-gray-200" style="color:unset; width:100%">
              <thead>
                <tr
                    ...
                    <th wire:click="sortBy('services.name')" class=" cursor-pointer">Service Name<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" width="15px" height="15px" style="display: inline;" fill="currentColor">
                            <path fill-rule="evenodd" d="M10 3a1 1 0 01.707.293l3 3a1 1 0 01-1.414 1.414L10 5.414 7.707 7.707a1 1 0 01-1.414-1.414l3-3A1 1 0 0110 3zm-3.707 9.293a1 1 0 011.414 0L10 14.586l2.293-2.293a1 1 0 011.414 1.414l-3 3a1 1 0 01-1.414 0l-3-3a1 1 0 010-1.414z" clip-rule="evenodd" />
                          </svg></th>
                    ...
                    <th wire:click="sortBy('icon')" class="px-4 py-3 cursor-pointer"> Icon </th>
                    <!-- <th wire:click="sortBy('detail')" class="px-4 py-3 cursor-pointer"> Detail</th> -->
                    <th class="px-4 py-3 xl:flex">Action</th>
                </tr>
            </thead>  
                <tbody class="bg-transperent divide-y dark:divide-gray-700">
                    @foreach ($servicecategories as $servicecategorys)
                        <tr class="text-xs font-semibold tracking-wide text-left text-gray-500 border-b dark:border-gray-700 bg-gray-50 dark:text-gray-400 dark:bg-gray-800">
                            <td class="px-4 py-3 break-all"> {{ $servicecategorys->id }} </td>
                            <td class="break-all"> {{ $servicecategorys->service_name }} </td>
                            <td class="px-4 py-3 break-all"> {{ $servicecategorys->name }} </td>
                            <td class="px-4 py-3">  @if($servicecategorys->icon != null)<img src="{{ $servicecategorys->icon }}" width="100" height="100" />@else Not uploaded @endif</td>
                            <!-- <td class="px-4 py-3"> <textarea disabled width="100" height="150">{{ $servicecategorys->detail }} </textarea></td> -->
                            <td>
                                <div class="flex space-x-1">&nbsp;&nbsp;
                                  <button wire:click="edit({{ $servicecategorys->id }})" id="edit" class="p-1 text-blue-600 hover:bg-blue-600 hover:text-white rounded">
                                        <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path d="M13.586 3.586a2 2 0 112.828 2.828l-.793.793-2.828-2.828.793-.793zM11.379 5.793L3 14.172V17h2.828l8.38-8.379-2.83-2.828z"></path></svg>
                                  </button>
                                  
                                    <button wire:click="confirmDelete({{ $servicecategorys->id }})" class="deleteServicecategory p-1 text-red-600 hover:bg-red-600 hover:text-white rounded">
              ...
              <div class="dark:bg-gray-800 p-2 sm:flex items-center justify-between">
                ...
                <div class="dark:bg-gray-800 my-4 sm:my-0">
                  <div  class="dark:bg-gray-800 lg:flex justify-center">
                      <span class="dark:bg-gray-800">{{ $servicecategories->links('livewire.datatables.tailwind-pagination') }}</span>
                  </div>
                </div>

                <div class="flex justify-end text-gray-600 dark:bg-gray-800">
                Service Categories {{ $servicecategories->firstItem() }} - {{ $servicecategories->lastItem() }} of
        ...
        <div class="absolute top-0 left-0 flex items-center justify-center w-full h-full" style="background-color: rgba(0,0,0,.5);" x-show="open"  >
          ...
          <div class="h-auto p-4 mx-2 text-left bg-white rounded shadow-xl md:max-w-xl md:p-6 lg:p-8 md:mx-0" @click.away="open = false">
            <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
              ...
              Are you sure want to delete this Service Category? 
              </h3>
            ...
            <div class="mt-5 sm:mt-6">
              <span class="flex rounded-md shadow-sm">
                ...
                <button wire:click="servicecategoryDelete()" class="inline-flex ml-4 justify-center w-full px-4 py-2 text-white bg-red-500 rounded hover:bg-red-700">
                    Delete!
                </button>
              </span>
            </div>
