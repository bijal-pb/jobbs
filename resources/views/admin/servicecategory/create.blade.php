...
<form wire:submit.prevent="store" enctype="multipart/form-data">    
    <div class="px-4 py-3 mb-8 bg-white rounded-lg shadow-md dark:bg-gray-800">
        <label class="block text-sm">
            ...
            <select  wire:model="serviceId" class="block w-full mt-1 text-sm dark:text-gray-300 dark:border-gray-600 dark:bg-gray-700 form-select focus:border-blue-400 focus:outline-none focus:shadow-outline-blue dark:focus:shadow-outline-gray">
            <option>Please select Service</option>
                @foreach($services as $service)
                    <option value="{{$service->id}}">{{ $service->name }}</option>
                @endforeach   
             </select>
             @error('serviceId') <span class="error text-red-500 text-xs italic">The select service field is required</span> @enderror
        </label>
        <label class="block text-sm">
        ...
        <label class="block text-sm">
            ...
            <!-- @error('icon') <span class="error text-red-500 text-xs italic">{{ $message }}</span> @enderror -->
        </label>
        <label class="block text-sm">
            ...
            <!-- @error('detail') <span class="error text-red-500 text-xs italic">{{ $message }}</span> @enderror -->
        </label>
        ...
        <div class="sm:w-full md:w-1/2 lg:w-1/2 xl:w-1/2 h-12 text-center align-baseline">
                ...
                <button class="inline-block align-baseline bg-blue-500 text-white py-2 px-4 rounded cursor-pointer" wire:click="cancel()">
                    Cancel
                </button>
            </div>
        </div>
